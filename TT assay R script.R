...
csv_file_list <- list.files(path = Experiment_dir,
                            ...
                            pattern = "\\.csv$",
                            full.names = TRUE) %>%
  # Filter out files that are in directories named "Results_"
...
merged_results <- merged_results %>%
  mutate(Genotype = if_else(str_detect(File_name, '823'), 'WT', 'KO'),
         ...
         Time_point_h = if_else(str_detect(File_name, 'mock'), '0', NA_character_),
         Time_point_h = ifelse(str_detect(File_name, '\\s\\d+h'), 
                               str_extract(File_name, '(?<=\\s)\\d+(?=h)'), 
                               Time_point_h),
         Treatment = case_when(
           str_detect(File_name, 'AZD') ~ 'AZD',
           str_detect(File_name, 'no N') ~ 'no_N',
...
KO_mean <- merged_results %>%
  filter(Genotype == "KO") %>%
  group_by(Organ, Time_point_h, Treatment) %>%
  summarise(Mean_ratio_for_KO = mean(Fluorescence_ratio_of_RFP_to_GFP, na.rm = TRUE), .groups = 'drop')
...
# Write the normalized data to a CSV file in the subfolder
...
# Write the WT only data to a CSV file in the subfolder
...
perform_analysis <- function(data, treatment, output_dir) {
  # Filter the data by treatment
  filtered_data <- data %>%
    filter(Treatment == treatment)
  
  # Run analysis of variance by Two-way ANOVA
  Two_way_Anova <- aov(Log_Normalized_ratio ~ Time_point_h * Organ, data = filtered_data)
  ...
  Tukey_with_CLD <- filtered_data %>%
    group_by(Time_point_h, Organ) %>%
    summarise(mean = mean(Log_Normalized_ratio), quant = quantile(Log_Normalized_ratio, probs = 0.75)) %>%
    arrange(desc(mean))
  
  # Add CLD to the Tukey table
  cld_df <- as.data.frame.list(cld$`Time_point_h:Organ`)
  Tukey_with_CLD$cld <- cld_df$Letters
  
  # Write the Tukey table with CLD to a CSV file in the subfolder
  write.csv(Tukey_with_CLD, file.path(output_dir, paste0("Tukey_with_CLD_", treatment, ".csv")))
  
  # Build box plots for WT ratios
  p1<-ggplot(WT_only, aes(Time_point_h, Fluorescence_ratio_of_RFP_to_GFP, fill = Organ)) +
    ...
    labs(title = paste("WT, RFP/GFP ratio -", treatment), x = "Elapsed time (h)", y = "RFP/GFP ratio")
  ggsave(file.path(output_dir, paste0("Box_plot_ratio_", treatment, ".pdf")), width = 5, height = 4)
 
   # Print the plot to RStudio’s Plots pane
  print(p1)
  
  # Build box plot normalized WT ratios
  p2<-ggplot(WT_only, aes(Time_point_h, Normalized_ratio, fill = Organ)) +
    geom_boxplot(width = 0.5, size = 0.15, outlier.shape = 1, outlier.size = 1) +
    theme_bw() +
    ...
    theme(panel.grid.major = element_line(size = 0.1), panel.grid.minor = element_line(size = 0.1)) +
    scale_fill_manual(values = c("#83D40F", "#EA9A16")) +
    labs(title = paste("WT, RFP/GFP ratio normalized to KO -", treatment), x = "Elapsed time (h)", y = "Normalized RFP/GFP ratio")
  ggsave(file.path(output_dir, paste0("Box_plot_normalized_ratio_", treatment, ".pdf")), width = 5, height = 4)
  # Print the plot to RStudio’s Plots pane
  print(p2)
}
...
# Perform analysis for each unique treatment
for (treatment in unique_treatments) {
  if (!is.na(treatment)) {
    perform_analysis(WT_only, treatment, output_dir)
  }
}
